...

{
  pa <- Conceptual_fig %>%
    ...
    geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf), data = ~ subset(., environment %in% c("Env_6")), colour = "grey70", size = 1, fill = NA, inherit.aes = F) +
...

{
  ...
  pc<-data.frame(x = SR_pc$sIV,
                 y = SR_pc$uIV,
                 z = SR_pc$mean_SR)%>%
                 #z = c(rep("PKwiIV",6), rep("PKwoIV",6), "PPK")) %>% 
    ggplot(aes(x, y, z))+
    ggplot2::scale_colour_viridis_c()+
    #scale_color_manual(values = c("grey70", "dodgerblue4", "goldenrod1")) +
    geom_polygon(data = data.frame(x=c(10.5,16,16,10.5),y=c(89,89,-5,-5)), color="grey70", fill = NA) + # objective 2
    geom_polygon(data = data.frame(x=c(64,69.5,69.5,64),y=c(35,35,-5,-5)), color="grey70", fill = NA) +
    geom_polygon(data = data.frame(x=c(-10,0,110,100),y=c(105,105,-5,-5)), color="dodgerblue4", fill = NA) + # objective 1
    geom_point(size=2, alpha=0.7, aes(color = z), show.legend=F)+
    xlab("sIV (%)")+
    ylab("uIV (%)")+
